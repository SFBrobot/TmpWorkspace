...
void setFltDiffDFlt(FltDiff *fDiff, float flt) { fDiff->flt = pow(.1, flt / 10 - 5); }

void resetFltDiff(FltDiff *fDiff, float value) {
	resetDiff(fDiff->diff, value);

	fDiff->out = fDiff->diff.out; //Defer to resetDiff for value handling
}

float updateFltDiff(FltDiff *fDiff, float input, float dt) {
	updateDiff(fDiff->diff, input, dt);

	return fDiff->out += (1 - pow(.5, dt * fDiff->flt)) * (fDiff->diff.out - fDiff->out);
}

#else #error rkControl diff has already been included!
#endif
