...
#pragma config(Motor,  port6,           trFly,         tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port7,           brFly,         tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port8,           blFly,         tmotorVex393TurboSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port9,           tlFly,         tmotorVex393TurboSpeed_MC29, openLoop, reversed, encoderPort, I2C_1)
...
#include "rkCompetition003.h"

#define DISMISS_BATT_WARNING_COND nLCDButtons
...
task lcd() {
	...
	resetDLatch(&dismissWarningLatch, 0);
	...
	while (true) {
		...
		if (rkBotDisabled || forceBattWarning) {
			bLCDBacklight = warningFlash;

			if (warningFlash)
				displayLCDCenteredString(0, "BATTERY WARNING:");
		...
		if (forceBattWarning && fallingEdge(&dismissWarningLatch, DISMISS_BATT_WARNING_COND)) {
			forceBattWarning = false;
			continue;
		}
	}
}
