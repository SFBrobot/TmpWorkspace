...
task _lcd() {
	...
	bool dispPwr = false,
		...
		centerBtn,
		pwrBtnsDown;
	long flashTS, btnTS, dispTS, time;

	DLatch btnRel, pwrBtnLatch;

	while (true) {
		time = time1[lcdTimer];
		centerBtn = nLCDButtons & kButtonCenter;
		...
		if (nImmediateBatteryLevel < battThresh) {
			...
			if (forceWarning || ((bIfiRobotDisabled || bIfiAutonomousMode) && !centerBtn)) {
				if (flash) displayLCDCenteredString(0, "BATTERY WARNING:");

				sprintf(str, "%.2f", nImmediateBatteryLevel / 1000.);
				displayLCDCenteredString(1, str);

				SensorValue[redLed] =
					SensorValue[yellowLed] =
					SensorValue[greenLed] =
					bLCDBacklight = flash;

				if (fallingEdge(&btnRel, toBool(nLCDButtons)) || time1[lcdTimer] > 10000) forceWarning = false;
		...
		if (bIfiRobotDisabled || bIfiAutonomousMode) {
			displayLCDCenteredString(0, "Mooseebot Mk. II");
			displayLCDCenteredString(1, "4800Buckets");
		}
		else {
			pwrBtns = twoWay((nLCDButtons & kButtonLeft) || vexRT[Btn7D], -flyPwrIncrement, (nLCDButtons & kButtonRight), flyPwrIncrement);

			pwrBtnsDown = pwrBtns != 0;

			if (pwrBtnsDown) {
				if (risingEdge(&pwrBtnLatch, pwrBtnsDown) {
